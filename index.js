...
setTitle("[TO BuY .gg/ft]  Lifetime Vanity SNIPER v1.0.1 31");
...
rl.question("> ", async (answer) => {
  switch (answer) {
    case '1':
      ...
      bot.on('rawWS', async (packet) => {
        if (packet.t === "READY") {
          ...
          logMessage('Gateway', `Logged in as 0.1ms#0`);
          ...
          const batchedData = Object.entries(Data).reduce((batches, [key, value]) => {
            console.log(`${clc.yellow('Server Vanity :')} [${clc.blue.bold(value[0])}] || ${clc.yellow(`Server Name :`)} [${clc.blue.bold(guilds[value[1]].name)}] || ${clc.yellow(`Server Id :`)} [${clc.blue.bold(guilds[value[1]].id)}]`);
      ...
      async function tryClaimVanity(value) {
        try {
          ...
          await axios.patch(
            `https://canary.discord.com/api/v7/guilds/${guild}/vanity-url`,
            { code: value[0] },
    ...
    case '3':
      ...
      break;
    case '4':
      console.log("Running Discord Bot module...");
      console.log("We are currently working on it.");
      break;
    case '5':
